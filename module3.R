...
Explore_beta0_first <- function(beta_sub, W, prevalence) {
  seq_beta0 <- seq(-20, 0, 0.01)
  seq_beta <- cbind(
    seq_beta0, beta_sub[1],beta_sub[2], beta_sub[3]
  )
  ...
  while (i <= iteration) {
    ...
    if (easy_separation_check1 != 0){
      ...
      if (easy_separation_check2 == 0){
        ...
        # Separation --------------------------------------------------------------
        # fit_separation <- glm(y ~ x + z1, data = df, family = binomial(link = "logit"),
