...
source("04_Script/Functions/module1_simple.R")

set.seed(1234)
iteration <- 2000
# n <- 1000
n <- c(1000, 10000, 100000)
# num_increase <- c(5, 10, 20, 30)
num_increase <- c(0, 1, 2, 5, 10, 20, 30, 50)
prob.x <- 0.5
...
write_rds(df_result, "03_Output/result_module1_240208.rds")
...
df_RD_naive <- df_result %>% 
  select(n, prevalence, RD_true, RR_true, n_separation, RD_ols, SE_ols) %>% 
  rename(est = RD_ols) %>% 
  rename(SE = SE_ols) %>% 
  mutate(method = "RD + naive SE")

df_RD_robust <- df_result %>% 
  select(n, prevalence, RD_true, RR_true, n_separation, RD_ols, SE_ols_robust) %>% 
  rename(est = RD_ols) %>% 
...
df_RR_poisson <- df_result %>% 
  ...
  select(n, prevalence, RD_true, RR_true, n_separation, est, SE, method)
...
fig_RD_bias <- ggplot(tab_RD, aes(x = RD_true, y = bias, color = method)) + 
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(position = pd) +
  geom_line(position = pd) +
  scale_x_continuous(breaks = c(0, 0.001, 0.002, 0.005, 0.01)) +
  scale_y_continuous(labels = label_number()) +
  labs(title = paste0("Prevalence: ", tab_RD$prevalence)) +
  theme_bw() +
  theme(panel.grid.minor = element_blank())
fig_RD_bias

fig_RD_rmse <- ggplot(tab_RD, aes(x = RD_true, y = rmse, color = method)) + 
  # geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(position = pd) +
  geom_line(position = pd) +
  scale_x_continuous(breaks = c(0, 0.001, 0.002, 0.005, 0.01)) +
...
fig_RR_est <- tab_RR2 %>% 
  ...
  scale_y_continuous(breaks = c(1,2,3,6,11,21,31), limits = c(0,40)) +
  labs(title = paste0("Prevalence: ", tab_RR$prevalence),
...
fig_RR_bias <- ggplot(tab_RR, aes(x = RR_true, y = bias_sep, color = method)) + 
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(position = pd) +
  geom_line(position = pd) +
  scale_x_continuous(breaks = c(1,2,3,6,11)) +
  scale_y_continuous(labels = label_number()) +
  labs(title = paste0("Prevalence: ", tab_RR$prevalence),
       subtitle = "Excluded separation cases") +
  theme_bw() +
  theme(panel.grid.minor = element_blank())
fig_RR_bias

fig_RR_rmse <- ggplot(tab_RR, aes(x = RR_true, y = rmse_sep, color = method)) + 
  # geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(position = pd) +
  geom_line(position = pd) +
  scale_x_continuous(breaks = c(1,2,3,6,11)) +
